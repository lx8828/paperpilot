...
_SYS_L1 = (
    ...
    "判断它们是否足以准确回答用户问题；若不足，从候选章节里指出最可能藏答案的 1~2 节。"
    "只输出 JSON。"
)
...
请判断：
1. 仅凭上述主张能否**准确**回答？能 → enough=true，target_sections=[]。
...
请判断：
- 若这些正文**直接包含**能作答的原句 → enough=true。
- 若明显缺失（答案在这段之外 / 数值对不上 / 该节不含此内容）→ enough=false，gap 说明还缺什么。
不要因为"理论上原文更权威"而无依据地判不够——若眼前正文已给出答案就判够。
{_json}"""
...
_SYS_L3 = (
    ...
    "判断能否据此**准确**回答；若全文都检索不到相关内容，enough=false（将诚实告知用户）。"
    "只输出 JSON。"
)
...
请判断：
- 若这些段落**直接包含**能作答的原句 → enough=true。
- 若检索结果与问题无关或不足以作答 → enough=false，gap 用一句话说明缺什么
  （这段话会直接呈现给用户，请写得像人话）。
{_json}"""
