"""实验：judge_l2_strict + 多候选圆心 组合 on L2 目标题群 67 题。

认知（2026-09-08 三轮）：净亏题 = 圆心错(10%命中) × 判够早停 × 单块窗口不足以拼全答案。
单独 strict(+4, 4题靠L3兜底) / 单独 multicent(=base, 圆心对了但窗口容量不足也答不好)。
组合 = strict 判不够时用多候选圆心在 L2 内扩到对块（省 L3）+ 松不开口。
本臂：build 完整漏斗图 judge_l2=judge_l2_strict + env PAPERPILOT_L2_MULTI_CENTER=1。
对照历史：base 40 / strict 44 / B 44 / multicent 40。
"""
...
PASS = 4
OUT = Path("qa/recall/ab_l2comb_result.json")
...
def main() -> int:
    ...
    for i, it in enumerate(items, 1):
        ...
        print(f"[{i}/{len(items)}] {qid[:10]} comb={score}({level},{u['calls']}c) "
    ...
    print("=" * 60)
    print(f"comb(strict+multicent) pass {ps}/{len(recs)}  prompt/题 {pt:.0f}  calls/题 {ca:.1f}")
